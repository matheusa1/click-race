      ...
      <div className="relative z-10">
        ...
        <div className="bg-slate-900/80 backdrop-blur-sm rounded-xl p-8 shadow-2xl border border-white/10 mb-8">
          ...
          <div className="flex justify-center items-center gap-4 mb-6">
            <div className={`w-3 h-3 rounded-full ${isConnected ? 'bg-green-500 animate-pulse shadow-green-500/50' : 'bg-red-500 shadow-red-500/50'} shadow-lg`} />
            ...
            <div>
                ...
                <div className="mb-8">
                  ...
                  <div className="flex justify-center gap-8 flex-wrap">
                    {gameState.players.map((player, index) => (
                      ...
                      >
                        <div className="text-slate-300 text-sm mb-1">Player {index + 1}</div>
                        <div className="text-blue-300 font-semibold text-base mb-1">{player.id}</div>
                        <div className="text-white font-bold text-lg">{player.clicks}</div>
              ...
              {gameState.status === EGameStatus.FINISHED && gameState.winner && (
                <div className="border-2 border-green-500 p-6 mt-4 rounded-xl bg-gradient-to-r from-green-500/10 to-emerald-500/20 text-center backdrop-blur-sm">
                  <h3 className="text-green-400 text-2xl mb-4 m-0">🏆 Partida Finalizada!</h3>
                  <p className="text-green-400 text-xl m-0 font-bold">
                    Vencedor: {gameState.winner.id} com {gameState.winner.clicks} cliques!
                  </p>
                </div>
              )}
            </div>
        ...
        {gameState && gameState.players.length > 0 && (
          <div className="bg-slate-900/80 backdrop-blur-sm rounded-xl p-8 shadow-2xl border border-white/10">
            <h3 className="text-slate-200 text-xl mb-6 text-center">🏁 Progresso da Corrida</h3>
              ...
              {gameState.players.map((player) => (
                <div key={player.id} className="flex flex-col items-center gap-2">
                  <div
                    ...
                    style={{ transform: `translateY(-${Math.min(player.clicks * 3, 100)}px)` }}
                  >
                    {player.id}
                  </div>
